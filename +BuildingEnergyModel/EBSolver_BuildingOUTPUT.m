...
elseif  Heat_on==1
    ...
    WasteHeat.TotAnthInput_URB      = (Hequip + Hpeople + LEequip + LEpeople)*Wroof/(Wcan+Wroof);  % W/m^2 urban area
...
if EnergyUse.EnergyForAC<0;
    Test=1;
end
if EnergyUse.EnergyForAC_H<0;
    Test=1;
end
...
% Sensible heat fluxes, (W/m^2) respective surface area
% -----------------------------------------------------
HbuildInt.HBinRoof      = HBinRoof;     % (W/m^2) interior roof area
HbuildInt.HbinWallSun   = HbinWallSun;  % (W/m^2) interior sunlit wall area
HbuildInt.HbinWallshd   = HbinWallshd;  % (W/m^2) interior shaded wall area
HbuildInt.HBinGround    = HBinGround;   % (W/m^2) interior ground area
HbuildInt.HbinIntMass   = HbinIntMass;  % (W/m^2) internall mass area (=Wall area)
HbuildInt.HbuildInSurf  = HbuildIn;     % (W/m^2) building ground area
HbuildInt.Hvent         = Hvent;        % (W/m^2) building ground area
HbuildInt.Hequip        = Hequip;       % (W/m^2) building ground area
HbuildInt.Hpeople       = Hpeople;      % (W/m^2) building ground area
...
LEbuildInt.LEequip      = LEequip;      % (W/m^2) building ground area
LEbuildInt.LEpeople     = LEpeople;     % (W/m^2) building ground area
LEbuildInt.LE_AC_Heat   = LE_AC_Heat;   % (W/m^2) building ground area
LEbuildInt.dSLE_air     = dSLE_air;       % (W/m^2) building ground area

% Conductive heat fluxes , (W/m^2) respective surface area
% -----------------------------------------------------
GbuildInt.G2Roof            = G2Roof;           % (W/m^2) interior roof area
...
% -----------------------------------------------------
esat_Tbin	=	611*exp(17.27*(Tbin-273.15)/(237.3+(Tbin-273.15)));	% Saturation vapor pressure of air in building interior [Pa]
e_bin		=	qbin*Pre/(0.622+0.378*qbin);	% Vapour pressure of air in building interior [Pa]
RH_bin		=	e_bin/esat_Tbin;				% Relative humidity of air in building interior [-]

HumidityBuilding.qbin       = qbin;         % Specific humidity of air in building interior [kg/kg]
HumidityBuilding.esatbin    = esat_Tbin;    % Saturation vapor pressure of air in building interior [Pa]
HumidityBuilding.ebin       = e_bin;        % Vapour pressure of air in building interior [Pa]
HumidityBuilding.RHbin      = RH_bin;       % Relative humidity of air in building interior [-]

% Time varying AC parameters
ParACHeat.AC_on         = AC_on;
ParACHeat.AC_onCool     = AC_onCool;
ParACHeat.AC_onDehum    = AC_onDehum;
ParACHeat.Heat_on       = Heat_on;
...
% Sensible heat fluxes, (W/m^2) respective surface area
% -----------------------------------------------------
HbuildInt.HBinRoof = 0; HbuildInt.HbinWallSun = 0; HbuildInt.HbinWallshd = 0;
HbuildInt.HBinGround = 0; HbuildInt.HbinIntMass = 0; HbuildInt.HbuildInSurf = 0;
HbuildInt.Hvent = 0; HbuildInt.Hequip = 0; HbuildInt.Hpeople = 0;
HbuildInt.H_AC_Heat = 0; HbuildInt.dSH_air = 0;

% Latent heat fluxes, (W/m^2) respective surface area
% -----------------------------------------------------
LEbuildInt.LEvent = 0; LEbuildInt.LEequip = 0; LEbuildInt.LEpeople = 0;
LEbuildInt.LE_AC_Heat = 0; LEbuildInt.dSLE_air = 0;

% Conductive heat fluxes , (W/m^2) respective surface area
% -----------------------------------------------------
GbuildInt.G2Roof = 0; GbuildInt.G2WallSun = 0; GbuildInt.G2WallShade = 0;
GbuildInt.Gfloor = 0; GbuildInt.dSinternalMass = 0;

% Humidity in building interior
% -----------------------------------------------------
esat_Tbin =	0; e_bin = 0; RH_bin = 0;
HumidityBuilding.qbin = qbin; HumidityBuilding.esatbin = esat_Tbin;
HumidityBuilding.ebin = e_bin; HumidityBuilding.RHbin = RH_bin;

% Time varying AC parameters
ParACHeat.AC_on = 0; ParACHeat.AC_onCool = 0; ParACHeat.AC_onDehum = 0;
ParACHeat.Heat_on = 0;
end
